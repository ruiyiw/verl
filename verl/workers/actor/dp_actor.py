...
from verl.trainer.ppo.core_algos import agg_loss, compute_ppo_policy_loss, compute_reinforce_policy_loss, kl_penalty
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    # )
                    # Modified by Ruiyi Wang (04/29/2025)
                    if self.config.policy_loss == "ppo":
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_ppo_policy_loss(
                            old_log_prob=old_log_prob,
                            log_prob=log_prob,
                            advantages=advantages,
                            response_mask=response_mask,
                    ...
                    elif self.config.policy_loss == "reinforce":
                        pg_loss = compute_reinforce_policy_loss(
                            log_prob=log_prob,
                            advantages=advantages,
                            response_mask=response_mask,
                            loss_agg_mode=loss_agg_mode,
                        ...
                        raise NotImplementedError(f"Policy loss not specified of unknown")
